...
class ManosFuturi:
    def __init__(self, root):
        ...
        self.overlay = np.array(Image.open(r'OutterFrame.png').convert("RGBA"))
        ...
        self.test_button = tk.Button(root, text="Test", command=self.run_gesture_test, bg="#7B68EE", fg="white",
                                     font=("Arial", 12, "bold"))
        self.test_button.place(x=585, y=535, width=65, height=40)
    ...
    def create_new_session_folder(self):
        session_num = 1
        while os.path.exists(os.path.join(self.save_dir, f"session{session_num}")):
            session_num += 1
        session_path = os.path.join(self.save_dir, f"session{session_num}")
    ...
    def enable_dragging(self, window):
        ...
        def on_drag(event):
            window.geometry(f"+{window.winfo_x() + (event.x - window.offset_x)}+{window.winfo_y() + (event.y - window.offset_y)}")
    ...
    def take_screenshot(self):
        ...
        if ret:
            frame = cv2.cvtColor(cv2.flip(frame, 1), cv2.COLOR_BGR2RGB)
            frame_resized = cv2.resize(frame, (800, 600))
            screenshot_path = os.path.join(self.session_folder, f"{self.pose_count + 1}.png")
            Image.fromarray(frame_resized).save(screenshot_path)
    ...
    def run_gesture_test(self):
        try:
            script_path = os.path.join(os.path.dirname(__file__), "gestures.py")
            subprocess.Popen([sys.executable, script_path])
        except Exception as e:
            print(f"Failed to run gestures.py: {e}")
